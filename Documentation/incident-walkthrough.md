...
Initial review of the account confirmed that existing authentication methods were invalid or misconfigured.
...
To restore access securely, the following steps were taken:

Forced MFA re-registration to clear invalid authentication methods

Issued a Temporary Access Pass (TAP) to enable secure recovery

Guided user to re-register MFA on next sign-in
...
Verified MFA flow was functional and user access restored
